...
def runCmd(args):
    try:
        usage_start = cnt()
        p1 = subp.run(args,capture_output=True,text=True,cwd=FILES_DIR,timeout=5)
...
def execute(args,inputs):
    try:
        usage_start = cnt()
        p1 = subp.Popen(args, stdout=subp.PIPE, stderr=subp.PIPE, stdin=subp.PIPE, text=True, cwd=FILES_DIR)
        for i in inputs:
            inp = str(i) + '\n'
            p1.stdin.write(inp)
            p1.stdin.flush()
...
if __name__ == "__main__":
    try:
        ...
        if lang == "c":
            ...
            if len(inputs) == 0:
                runCmd(['go', 'run', f'{fileID}.go'])
            else:
                execute(['go', 'run', f'{fileID}.go'], inputs)
        elif lang == "cpp":
